...
export function raycastForQoderURI(uri: string) {
  return `qoder://tonka3000.raycast/${uri}`;
}

export async function openURIinQoder(uri: string) {
  await open(raycastForQoderURI(uri));
}
