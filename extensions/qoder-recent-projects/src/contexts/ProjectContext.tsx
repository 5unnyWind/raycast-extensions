...
import { run } from "../integrations/qoder-directory";
...
export function ProjectProvider({ children, launchContext }: { children: ReactNode; launchContext?: LaunchContext }) {
  const openProject = async (uri: string, closeOtherWindows: boolean) => {
    try {
      ...
      await open(uri, "Qoder");

      const { qoderDirectory, callbackLaunchOptions } = launchContext || {};

      if (qoderDirectory && qoderDirectory.ruleContent) {
        await run(uri, {
          ruleContent: qoderDirectory.ruleContent,
          replace: qoderDirectory.replace,
        });
      }
