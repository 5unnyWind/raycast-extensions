...
export function useRecentEntries() {
  ...
  async function removeEntry(entry: EntryLike) {
    ...
    try {
      ...
      await revalidate();
      showToast(Toast.Style.Success, "Entry removed", `Restart Qoder to sync the list in Qoder (optional)`);
    } catch (error) {
  ...
  async function removeAllEntries() {
    try {
      if (
        ...
        await saveEntries([]);
        await revalidate();
        showToast(Toast.Style.Success, "All entries removed", `Restart Qoder to sync the list in Qoder (optional)`);
      }
    } catch (error) {
...
function getPath() {
  return `${homedir()}/Library/Application Support/Qoder/User/globalStorage/state.vscdb`;
}
